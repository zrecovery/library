...
import {
  ...
  NotFoundError,
  UnknownError,
} from "../../../../domain/src/shared/domain";
import type { Logger } from "../../../../domain/src/shared/domain/interfaces/logger";
import {
  type StoreError,
  StoreErrorTag,
} from "../../../../domain/src/shared/domain/interfaces/store.error";
    ...
    switch (error._tag) {
      case StoreErrorTag.NotFound:
        return new NotFoundError(
          id
            ? `${entityNameCapitalized} not found: ${id}`
            : `${entityNameCapitalized} not found`,
        );
 ...
 */
export const withStoreResultHandling = <T, _E>(
  logger: Logger,
  entityNameLowercase: string,
  entityNameCapitalized: string,
  ...
  const transformError = createStoreResultErrorHandler(
    logger,
    entityNameLowercase,
...
export const createOperationLogger =
  ...
  <T>(params?: T): void => {
    logger.debug(
      `${operationName}: ${params ? JSON.stringify(params) : "Executing"}`,
    );
  };
