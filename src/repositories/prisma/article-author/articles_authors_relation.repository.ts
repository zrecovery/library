...
export class ArticleSAuthorsRelationPrismaRepository implements ArticleAuthorRelationshipRepository {
    #client: PrismaClient;
    ...
    update(id: number, updated: Partial<ArticleAuthorRelationship>): Promise<Required<ArticleAuthorRelationship>> {
        return this.#client.articles_authors.update({ where: { id }, data: { ...updated } });
    }
    ...
    list = async (query: Query): Promise<PaginatedResponse<Required<ArticleAuthorRelationship>[]>> => {
        ...
        const authorsArticlesRelationship = await this.#client.articles_authors.findMany({
            where: where,
            skip: offset,
            take: limit,
        });
