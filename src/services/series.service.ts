...
import { ISeriesDetail, PaginatedResponse } from "@src/interfaces/response.interface";
import { Series } from "@src/model";
import { ArticleRepository } from "@src/repositories/article.repository.port";
import { AuthorRepository, ArticleAuthorRelationshipRepository } from "@src/repositories/author.repository.port";
import { SeriesRepository, ChapterRepository } from "@src/repositories/series.repository.port";
    ...
    findById = async (id: number): Promise<ISeriesDetail> => {
        ...
        const articles = await Promise.all(
            chapters.detail.map(async (chapter) => {
                const article = (await this.#articleRepository.list({ chapter_id: chapter.id })).detail[0];
                return { order: chapter.order, ...article }
            })
        )
        const authors = await Promise.all((await Promise.all(
            articles.map(async (article) => {
                const authors = await this.#authorArticleRepository.list({ article_id: article.id, size: -1 });
                return authors.detail
            })
        ))
            .flatMap((a) => a)
            .map(async (authors_articles) => {
                return await this.#authorRepository.getById(authors_articles.author_id)
            }))
