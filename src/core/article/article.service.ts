...
export default class ArticleService {
    ...
    async getByAuthorId(authorId: number, limit: number, offset: number): Promise<Article[]> {
        return await this.articleRepository.getArticlesByAuthorId(authorId, limit, offset);
    }

    async getList(query: Query, limit: number, offset: number): Promise<Article[]> {
